...
To compile the project from GitHub with IntelliJ, perform the following steps:

\begin{enumerate}
	\item Clone the directory using the URL \textbf{https://github.com/cs383-4/sQuire.git}.
	\item Allow Maven to import changes and reindex files.
	\item Run the \textbf{SquireServer} run configuration which will compile and launch the server.
	\subitem The server is by default listening on port 3017.
	\item Run the \textbf{RunHello} run configuration which will compile and launch the client application.
\end{enumerate}

\newpage
\subsubsection{Dependencies (wern0096)}

The sQuire project relies on the following libraries:

\begin{itemize}
	\item Ebean ORM
	\item Ebean ORM and Type Query Generator
	\item Xerial SQLite Database Driver
	\item JUnit
	\item TestFX
	\item JavaFX
	\subitem This comes with the latest JDK.
	\item Google Mobwrite
	\subitem This package should come as part of the project.
\end{itemize}

You may also refer to the \textbf{pom.xml} file for project dependencies if not using Maven and then install the specified jars/libraries individually. Otherwise, simply allow Maven to import them.
...
For the server, we use \textbf{squire.Networking.Server} as the entry class, and \textfb{squire.Main} for the client. Then, we create a basic squire.db file using IntelliJ's built in SQLite capabilities using the generated sqlite-create-all.sql script.
...
\hline
File   & Size \\ \hline
server.jar   & 11MB     \\ \hline
client.jar   & 11MB     \\ \hline
\end{tabular}
\\ \noindent Both of these files have the same file size. This is because both of the JAR files have exactly the same code. See the Deployment section for more details. \\
\newpage
...
%\begin{itemize}
%	\item Integrating Mobwrite and syntax highlighting functionality.
%	\item Initially dividing work into proper tasks for indivduals.
%	\item
%
%\end{itemize}

\subsubsection{Integrating Mobwrite and Syntax Highlighting}
We never found an effective way to get syntax highlighting working with our Mobwrite components. Our most promising attempt involved a project called \textit{RichTextFX}. We dedicated a good number of hours to this issue, and ultimately had to abandon implementing the use cases involving syntax highlighting.
\subsubsection{Dividing Workload}
During our first few sprints, we were unsure how to divide the workload between team members. This resulted in some backlog items being assigned to too many people. Due to communication shortcomings, some tasks got implemented in two different ways. This redundant work set us back a little at times.
