...
\subsubsection{sQuire server}
The sQuire server is the part of the application that handles storing and retrieving information in the database. It consists of two main parts: The Router and the Server. The Server accepts socket connections one at a time in a queue. Upon receiving a connection, the server accepts a Request object, and then passes this object to the Router.
...
The client makes requests to the sQuire server only when doing things like creating new files or projects, browsing project, and logging in. Most of the collaborative part that requires continuous requests is handled by the MobWrite server. As such, this server won't be 
...
\newpage
\section[METRICS]{\selectlanguage{english}\rmfamily\bfseries\color{black} METRICS}
